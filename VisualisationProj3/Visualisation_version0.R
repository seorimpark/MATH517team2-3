...
server <- function(input, output) {
  ...
  output$Fires <- renderPlot({
    ...
    data_year<-filter_year(data_all,Year)
    p2 <- (data_year %>% 
  ...
  output$Temp <- renderPlot({
    ...
    templot <- (data_year %>% 
      ...
      scale_color_manual(name = "Temperature",
                         values = c("(-20,-10]" = "paleturquoise2",
                                    "(-10,0]" = "sky blue",
                                    "(0,10]" = "yellow",
                                    "(10,20]" = "darkgoldenrod2",
                                    "(20,30]" = "orange",
                                    "(30, 40]" = "red"),
                         labels = c("-20 < Temperature <= -10", "-10 < Temperature <= 0","0 < Temperature <= 10","10 < Temperature <= 20","20 < Temperature <= 30","30 < Temperature <= 40"))+
      theme_minimal() + 
      ggtitle('Temperature in USA in 2000')
    templot
