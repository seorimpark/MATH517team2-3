...
May 2012 was picked to visualise the distribution of the Temperature over the country. This month has been selected randomly. 
Figure \@ref(fig:TempVis) is displayed a gif containing three different graphs. First the distribution of the Temperatures across the country in May 2012. Then, the same plot but this time the number of fires was added on top. And lastly, the same plot as prior but this time the burnt area is added on top. 

We can see from the plot that the temperatures are higher reaching 28.15 degrees celsius near the border of Mexico. Whereas the coldest state is Montana with an average of 1.37 degrees celsius during the month of May. 

Looking at the number of fires, we can see that as a preliminary visual analysis, the fires occur in the hottest regions. The same way, the largest burnt area cluster in the hottest areas. More rigorous analysis will follow back. 
...
The same way as in Figure \@ref(fig:TempVis), Figure \@ref(fig:TempVis) represents the pressure distribution and their different attributes. One first strong observation when comparing Figures \@ref(fig:TempVis) and \@ref(fig:TempVis) is that the distribution of both meteorological factor are extremely similar. Indeed, areas with lower temperatures coicide with lower pressure and same applied in the converse situation. This is explained by the Ideal gas law which states that: 
$$P V = n R T $$
Where P, V and T are the pressure, volume and temperature; n is the amount of substance; and R is the ideal gas constant. We can see that the pressure and temperature are perfectly correlated. 

Conserning the number of fires and burnt areas, the same conclusion as for the temperature can be applied. 
...
In Figure \@ref(fig:WindVis) are displayed the wind directions and their intensities using arrows that differ in direction and length. Due to the large number of data points, only one arrow out of two is displayed. We first note that the wind comes from the south to north, both west and east to center. 

We can see from the plot that area with stronger wind : for example in the West coast and in the far south, fires are more predominant and the burnt area are more important. 

Note that by definition, wind results from a change of pressure in an area, thus the concentration of fires echoes the visual analysis made earlier for the pressure.   
...
```
In Figure \@ref(fig:EvaVis) is displayed the distribution of the evaporation across the country. Notice how again this distribution is strongly matching with the distribution seen in Figure \@ref(fig:TempVis). One can still notice that although the high temperatures in the West coast, the evaporation (of water) is much smaller than the one seen in Florida for example. This can come from the fact that we are singularizing one feature rather than lookin at the impact of all the features available with their interaction. 
