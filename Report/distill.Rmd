...
Year <- 2000
    ...
    p <- data_all %>% 
      mutate(`Major land cover`=factor(max.col(data_all %>% dplyr::select(starts_with("lc"))) ))%>% 
...
for( y in 1993:2015){
  df_temp<- data %>% 
  mutate(`Major land cover`=factor(max.col(data %>% dplyr::select(starts_with("lc"))), labels = names[1:18 %in% max.col(data  %>% dplyr::select(starts_with("lc")))])) %>% filter(year==y)
